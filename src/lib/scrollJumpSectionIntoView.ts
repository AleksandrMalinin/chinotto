 ...
 */
export const STREAM_SECTION_TOP_OFFSET_REM = 1.3;

/** Scroll top after jumping to `entryId`, or null when the target is not in the DOM. */
export function computeJumpScrollTop(entryId: string): number | null {
  ...
  return Math.min(maxScroll, Math.max(0, Math.ceil(y)));
}
...
export function scrollJumpSectionIntoView(entryId: string): void {
  const top = computeJumpScrollTop(entryId);
  if (top === null) return;
