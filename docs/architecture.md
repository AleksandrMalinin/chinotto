...
- **Optional Firestore sync:** When `VITE_FIREBASE_*` is set, the app can sign in with Apple (same Firebase project as mobile) and **pull + push** entries. **Desktop:** `docs/sync.md` (architecture, status, ops). **Wire contract:** `chinotto-mobile/docs/sync/sync.md`. **Release QA:** `docs/sync-release-checklist.md`. No cloud requirement for core capture/search.
