...
- **Packaged desktop (GitHub / website DMG):** **`native_apple_sign_in`** (Tauri) + **`signInWithAppleCredential`**. Notarized **Developer ID** releases use **`Chinotto.developer-id.entitlements`** in **`tauri.conf.json`** (microphone + audio only; no **`com.apple.developer.applesignin`**) so the app can launch on **macOS 26+** where AMFI may reject that entitlement without a matching provisioning profile. **Local team-signed** testing with full native SIWA still uses **`Chinotto.entitlements`** via **`scripts/codesign-macos-dev.sh`**. **Mac App Store** builds use **`Chinotto.mas.entitlements`** (**`scripts/build-mas-testflight.sh`**).  
...
| **App won’t open** — **RBSRequestErrorDomain** / **Launchd job spawn failed** (**163**) | Often **macOS 26+** + **Developer ID** + **`com.apple.developer.applesignin`** in the signed app. **Default DMG** builds omit that entitlement via **`Chinotto.developer-id.entitlements`**. If **`native_apple_sign_in`** then fails, use the **Mac App Store** build or **`scripts/codesign-macos-dev.sh`** for full SIWA. |
