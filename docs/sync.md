...
- **Packaged desktop (GitHub / website DMG):** **`native_apple_sign_in`** (Tauri) runs **Sign in with Apple** in-process; the Apple **ID token** + raw **nonce** are passed to **`signInWithAppleCredential`** (Firebase JS). No Safari loopback. Release signing uses **`Chinotto.entitlements`** (`tauri.conf.json`). **Mac App Store** builds use **`Chinotto.mas.entitlements`** (see **`scripts/build-mas-testflight.sh`**).  
...
| **App won’t open** — **RBSRequestErrorDomain** / **Launchd job spawn failed** (**163**) | Seen on **some macOS versions** (incl. **beta**) with **Developer ID** + certain entitlements while **`spctl`** still says **accepted** — OS / signing policy, not app logic. Try **stable macOS**, **MAS** build, or **Apple Developer** guidance for your **Team ID + App ID** + notarization. Optional local adhoc signing uses **`Chinotto.developer-id.entitlements`** (**`scripts/sign-macos.sh`**, mic only). |
